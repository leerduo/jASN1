...
public class PE_PINCodes {
	...
	public static class SubChoice_pinCodes {
		...
		public int encode(BerByteArrayOutputStream os, boolean explicit) throws IOException {
			...
			int codeLength = 0;
			if (filePath != null) {
				codeLength += filePath.encode(os, true);
				return codeLength;

			}
			
			if (pinconfig != null) {
				codeLength += pinconfig.encode(os, true);
				return codeLength;
		...
		public int decode(InputStream is, BerIdentifier berIdentifier) throws IOException {
			...
			if (berIdentifier.equals(SubSeqOf_pinconfig.identifier)) {
	...
	public int encode(BerByteArrayOutputStream os, boolean explicit) throws IOException {
		...
		if (code != null) {
			...
			codeLength += pin_Header.encode(os, true);
	...
	public int decode(InputStream is, boolean explicit) throws IOException {
		...
		if (length.val == -1) {
			...
			int choiceDecodeLength = pinCodes.decode(is, berIdentifier);
		...
		pinCodes = new SubChoice_pinCodes();
		subCodeLength += pinCodes.decode(is, berIdentifier);
		if (subCodeLength == length.val) {
			return codeLength;
		}
